...
[`references/terminology-sources.md`](references/terminology-sources.md)。
...
5. **兩岸用語在地化轉換（依據 moedict-data-csld 對照）**：
...
6. **語境多義詞判讀（Context-Aware Disambiguation）**：
