...
Agent 判斷需求可能跨單元重用、改變判定或改變輸出契約時，應主動摘要規則與影響，
請使用者選擇：

- **只套用本次**：完成當次工作，不建立永久規則。
- **記為候選需求**：先進本機品質紀錄；未經確認，不修改正式 Skill。
- **納入共通規則**：建立最小重現案例或去識別品質紀錄，先補測試，再修改資料或
  Skill，最後更新版本與 Changelog。

使用者已明確指定處理方式時不重複詢問。單次教材特例不應因一句對話直接升級成
共通規則。
